                ...
                <div class="row">
                    <div class="col-sm-6">
                        ...
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">Hakkımızda ekle</li>
                        </ol>
                    </div>
                        ...
                        <div class="card">
                            ...
                            <div class="card-body add-post">
                                ...
                                <form class="row needs-validation" method="post" autocomplete="off"
                                      enctype="multipart/form-data" action="{{route('hakkimizda.update',1635233954)}}">
                                    @method('PUT')
                                    @csrf
                                    <div class="col-sm-12">
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-12">
                                                ...
                                                @foreach($hakkimizda as $hakkimizdaa)
                                                    <div class="input-group-prepend">
                                                        <div class="input-group-text"><i style="margin: 10px" class="flag-icon flag-icon-{{$hakkimizdaa->lang}}"></i>
                                                            <input value="{{$hakkimizdaa->title}}" name="title_{{$hakkimizdaa->lang}}" class="form-control baslik{{$hakkimizdaa->id}}" id="validationCustom01" type="text" placeholder="{{$hakkimizdaa->langName}} Başlık Giriniz"></div>
                                                    </div>
                                                    <br>
                                                @endforeach
                                            </div>
                                            ...
                                            <div class="email-wrapper">
                                                <div class="theme-form">
                                                    <div class="form-group">
                                                        <i style="margin: 10px"
                                                           class="flag-icon flag-icon-{{$hakkimizdaa->lang}}"></i>
                                                        <label>İçerik:</label>
                                                        <textarea id="summernote{{$hakkimizdaa->id}}"
                                                                  name="text_{{$hakkimizdaa->lang}}">{!! $hakkimizdaa->text !!}</textarea>
                                                    </div>
                                                </div>
                                            </div>
                                        @endforeach
...
@section('js')
    ...
    <script>
        ...
        $(document).ready(function () {
            $('#summernote{{$hakkimizdaa->id}}').summernote({
                height: 300,
            });
        });
        @endforeach
    ...
    <script>
        @foreach($hakkimizda as $hakkimizdaa)
        function readURL{{$hakkimizdaa->id}}(input) {
            if (input.files && input.files[0]) {
                var reader = new FileReader();
                reader.onload = function (e) {
                    $('#imagePreview{{$hakkimizdaa->id}}').css('background-image', 'url(' + e.target.result + ')');
                    $('#imagePreview{{$hakkimizdaa->id}}').hide();
                    $('#imagePreview{{$hakkimizdaa->id}}').fadeIn(650);
                }
                reader.readAsDataURL(input.files[0]);
            }
        }

        $("#imageUpload{{$hakkimizdaa->id}}").change(function () {
            readURL{{$hakkimizdaa->id}}(this);
        });
        @endforeach
    </script>
    <script>
    ...
    <script>
        $('document').ready(function() {
            ...
            $(".bloc-text-image .delete").click(function() {
                ...
                $(this).parent().data('id');
                $.ajax({
                    url: "{{route('hakkimizda.imagedelete')}}",
                    method: "post",
                    headers: {
