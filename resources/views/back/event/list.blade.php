                            ...
                            <div class="bg-primary b-r-4 card-body">
                                <div class="media static-top-widget">
                                    ...
                                    <div class="media-body"><span class="m-0">Etkinlik Sayısı</span>
                                        <h4 class="mb-0 counter">{{$event->count()}}</h4><i class="icon-bg"
                        ...
                        <div class="card o-hidden border-0">
                            <div class="bg-danger b-r-4 card-body">
                                <div class="media static-top-widget">
                                    ...
                                    <div class="media-body"><span class="m-0">Deaktif Etkinlik</span>
                                        <h4 class="mb-0 counter">{{$event->where('status',0)->count()}}</h4><i class="icon-bg"
                        ...
                        <div class="card o-hidden border-0">
                            <div class="bg-primary b-r-4 card-body">
                                <div class="media static-top-widget">
                                    ...
                                    <div class="media-body"><span class="m-0">Aktif Etkinlik</span>
                                        <h4 class="mb-0 counter">{{$event->where('status',1)->count()}}</h4><i class="icon-bg"
                            ...
                            <div class="table-responsive">
                                <table class="display " id="basic-2">
                                    ...
                                    @foreach($event as $events)
                                        <tr class="tr-check" data-id="{{$events->id}}" data-status="{{$events->status}}">
                                            <td>{{$events->title}}</a></td>
                                            <td style="text-align: center"><img style="border-radius: 10%" class="img"
                                                                                src="{{asset("public/".$events->image)}}"
                                                                                height="100"></td>
                                            @foreach(DB::table('users')->where('id',$events->user_id)->get() as $uu)
                                            <td>{{$uu ->name}}</a></td>
                                            @endforeach
                                            <td>{{ \Carbon\Carbon::parse($events->created_at)->day }} {{ \Carbon\Carbon::parse($events->created_at)->monthName }} {{ \Carbon\Carbon::parse($events->created_at)->year }}</td>
                                            <td><label class="switch">
                                                    <input type="checkbox" class="checkk"
                                                           @if($events->status) checked @endif>
                                                    <span class="slider round"></span>
                                                </label>
                                            </td>
                                            <td style="text-align: center" >
                                                <a  target="_blank" href="{{route('event.edit',$events->id)}}" class="btn btn-pill btn-info-gradien" type="button"><i class="fa fa-eye"></i></a>
                                                <a href="{{route('event.destroy',$events->id)}}" class="btn btn-pill btn-danger-gradien" type="button"><i class="fa fa-trash"></i></a>
                                            </td>
                                        </tr>
                                    @endforeach
    ...
    <script>
        $(".checkk").change((e) => {
            ...
            $.ajax({
                ...
                success: function (d) {
                    toastr.success($(e.target).is(":checked") ? "Etkinlik Durumu Aktif" : "Etkinlik Durumu Pasif")
