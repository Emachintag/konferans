                ...
                <div class="row">
                    <div class="col-sm-6">
                        ...
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item">Haber</li>
                        ...
                        <div class="card">
                            ...
                            <div class="card-body add-post">
                                ...
                                <form class="row needs-validation" method="post" autocomplete="off"
                                      enctype="multipart/form-data" action="{{route('haber.update',$haber[0]->lang_id)}}">
                                    @method('PUT')
                                    @csrf
                                    ...
                                    <div class="col-sm-12">
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                @foreach($haber as $habers)

                                                    <div class="input-group-prepend">
                                                        <div class="input-group-text"><i style="margin: 10px"
                                                                                         class="flag-icon flag-icon-{{$habers->lang}}"></i>
                                                            <input  value="{{$habers->title}}"  name="title_{{$habers->lang}}" class="form-control baslik{{$habers->id}}"
                                                                    id="validationCustom01" type="text"
                                                                    placeholder="{{$habers->langName}} Başlık Giriniz"
                                                            ></div>
                                                    </div>
                                                    <br>
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                @foreach($haber as $habers)
                                                    <div class="input-group-prepend">
                                                        <div class="input-group-text"><i style="margin: 10px"
                                                                                         class="flag-icon flag-icon-{{$habers->lang}}"></i>
                                                            <input value="{{$habers->url}}" name="url_{{$habers->lang}}" class="form-control url{{$habers->id}}"
                                                                   id="slug-target" type="text"
                                                                   placeholder="{{$habers->langName}} URL Alanı Otomatik Oluşturuluyor"
                                                            ></div>
                                                    </div>
                                                    <br>
                                                @endforeach
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                @foreach($haber as $habers)
                                                    <div class="input-group-prepend">
                                                        <div class="input-group-text"><i style="margin: 10px"
                                                                                         class="flag-icon flag-icon-{{$habers->lang}}"></i>
                                                            <input value="{{$habers->title_2}}" name="title_2_{{$habers->lang}}" class="form-control"
                                                                   id="validationCustom01" type="text"
                                                                   placeholder="{{$habers->langName}} Alt Başlık Giriniz"
                                                            ></div>
                                                    </div>
                                                    <br>
                                                @endforeach
                                            ...
                                            <div class="col-md-6">
                                                <div>
                                                    ...
                                                    <select
                                                        ...
                                                        <optgroup label="Kategoriler">
                                                            @foreach($habercategorysingle as $habercategorysingles)
                                                                <option @if($habers->lang_id == $habercategorysingles->lang_id) selected @endif value="{{$habercategorysingles->id}}">{{$habercategorysingles->kategori}}</option>
                                                            @endforeach
                                                        </optgroup>
                                                    </select>
                                                </div>
                                                ...
                                                <div class="input-group-prepend">
                                                    <div class="input-group-text">
                                                        <input value="{{$habers->sira}}" name="sira" class="form-control" id="validationCustom01" type="number" placeholder="Sıra Giriniz">
                                                    </div>
                                                </div>
                                            </div>
                                                ...
                                                <div class="col-md-6">
                                                    <div class="avatar-upload">
                                                        <div class="avatar-edit">
                                                            <input name="image_{{$habers->lang}}" type='file' id="imageUpload{{$habers->id}}" accept=".png, .jpg, .jpeg" />
                                                            <label for="imageUpload{{$habers->id}}"></label>
                                                        </div>
                                                        <i style="margin: 10px"
                                                           class="flag-icon flag-icon-{{$habers->lang}}"></i>
                                                        <label>Haber Kapak Fotoğraf:</label>
                                                        <div class="avatar-preview">
                                                            <div id="imagePreview{{$habers->id}}" style="background-image: url({{asset('public/'.$habers->image)}});">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="theme-form">
                                                    <div class="form-group">
                                                        <i style="margin: 10px"
                                                           class="flag-icon flag-icon-{{$habers->lang}}"></i>
                                                        <label>İçerik:</label>
                                                        <textarea id="summernote{{$habers->id}}"
                                                                  name="text_{{$habers->lang}}">{!! $habers->text !!}</textarea>
                                                    </div>
                                                </div>
                                            </div>
                                        @endforeach
                                    ...
                                    @foreach(DB::table('haber_gorsel')->where('haber_id',$haber[0]->lang_id)->get() as $images)
                                    ...
                                    <input type="hidden" name="id" value="{{$haber[0]->lang_id}}" >
...
@section('js')
    ...
    <script>
        ...
        $(document).ready(function () {
            $('#summernote{{$habers->id}}').summernote({
                height: 300,
            });
        });
        @endforeach
    ...
    <script>
        @foreach($haber as $habers)
        function readURL{{$habers->id}}(input) {
            if (input.files && input.files[0]) {
                var reader = new FileReader();
                reader.onload = function (e) {
                    $('#imagePreview{{$habers->id}}').css('background-image', 'url(' + e.target.result + ')');
                    $('#imagePreview{{$habers->id}}').hide();
                    $('#imagePreview{{$habers->id}}').fadeIn(650);
                }
                reader.readAsDataURL(input.files[0]);
            }
        }

        $("#imageUpload{{$habers->id}}").change(function () {
            readURL{{$habers->id}}(this);
        });
        @endforeach
    </script>
    <script>
    ...
    <script>
        ...
        @foreach($haber as $habers)
        $(document).on("blur", '.baslik{{$habers->id}}', function (event) {
            var value = $(this).val().toString().trim() , slug = convertToSlug(value).toString().trim();
            if (value) {
                $('.url{{$habers->id}}').val(slug)
            }

        });

        $(document).on("blur", '.url{{$habers->id}}', function (event) {
            var value = $(this).val().toString().trim() , slug = convertToSlug(value).toString().trim();
            if (value) {
                $(this).val(slug)
            }
