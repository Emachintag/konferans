                        ...
                        <div class="card">
                            ...
                            <div class="card-body add-post">
                                ...
                                <form class="row needs-validation" method="post" autocomplete="off"
                                      enctype="multipart/form-data" action="{{route('urun.update',$urun[0]->lang_id)}}">
                                    @method('PUT')
                                    @csrf
                                    ...
                                    <div class="col-sm-12">
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                @foreach($urun as $uruns)

                                                    <div class="input-group-prepend">
                                                        <div class="input-group-text"><i style="margin: 10px"
                                                                                         class="flag-icon flag-icon-{{$uruns->lang}}"></i>
                                                            <input  value="{{$uruns->title}}"  name="title_{{$uruns->lang}}" class="form-control baslik{{$uruns->id}}"
                                                                    id="validationCustom01" type="text"
                                                                    placeholder="{{$uruns->langName}} Başlık Giriniz"
                                                            ></div>
                                                    </div>
                                                    <br>
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                @foreach($urun as $uruns)
                                                    <div class="input-group-prepend">
                                                        <div class="input-group-text"><i style="margin: 10px"
                                                                                         class="flag-icon flag-icon-{{$uruns->lang}}"></i>
                                                            <input value="{{$uruns->url}}" name="url_{{$uruns->lang}}" class="form-control url{{$uruns->id}}"
                                                                   id="slug-target" type="text"
                                                                   placeholder="{{$uruns->langName}} URL Alanı Otomatik Oluşturuluyor"
                                                            ></div>
                                                    </div>
                                                    <br>
                                                @endforeach
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                @foreach($urun as $uruns)
                                                    <div class="input-group-prepend">
                                                        <div class="input-group-text"><i style="margin: 10px"
                                                                                         class="flag-icon flag-icon-{{$uruns->lang}}"></i>
                                                            <input value="{{$uruns->title_2}}" name="title_2_{{$uruns->lang}}" class="form-control"
                                                                   id="validationCustom01" type="text"
                                                                   placeholder="{{$uruns->langName}} Alt Başlık Giriniz"
                                                            ></div>
                                                    </div>
                                                    <br>
                                                @endforeach
                                            ...
                                            <div class="col-md-6">
                                                    ...
                                                    <select
                                                        ...
                                                        <optgroup label="Kategoriler">
                                                            @foreach($uruncategorysingle as $uruncategorysingles)
                                                                <option @if($uruns->lang_id == $uruncategorysingles->lang_id) selected @endif value="{{$uruncategorysingles->id}}">{{$uruncategorysingles->kategori}}</option>
                                                            @endforeach
                                                        </optgroup>
                                                    </select>
                                                </div>
                                                ...
                                                <div class="input-group-prepend">
                                                    <div class="input-group-text">
                                                        <input value="{{$uruns->sira}}" name="sira" class="form-control" id="validationCustom01" type="number" placeholder="Sıra Giriniz">
                                                    </div>
                                                </div>
                                            </div>
                                                ...
                                                <div class="col-md-6">
                                                    <div class="avatar-upload">
                                                        <div class="avatar-edit">
                                                            <input name="image_{{$uruns->lang}}" type='file' id="imageUpload{{$uruns->id}}" accept=".png, .jpg, .jpeg" />
                                                            <label for="imageUpload{{$uruns->id}}"></label>
                                                        </div>
                                                        <i style="margin: 10px"
                                                           class="flag-icon flag-icon-{{$uruns->lang}}"></i>
                                                        <label>Ürün Kapak Fotoğraf:</label>
                                                        <div class="avatar-preview">
                                                            <div id="imagePreview{{$uruns->id}}" style="background-image: url({{asset('public/'.$uruns->image)}});">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="theme-form">
                                                    <div class="form-group">
                                                        <i style="margin: 10px"
                                                           class="flag-icon flag-icon-{{$uruns->lang}}"></i>
                                                        <label>İçerik:</label>
                                                        <textarea id="summernote{{$uruns->id}}"
                                                                  name="text_{{$uruns->lang}}">{!! $uruns->text !!}</textarea>
                                                    </div>
                                                </div>
                                            </div>
                                        @endforeach
                                    ...
                                    @foreach(DB::table('urun_gorsel')->where('urun_id',$urun[0]->lang_id)->get() as $images)
                                    ...
                                    <input type="hidden" name="id" value="{{$urun[0]->lang_id}}" >
...
@section('js')
    ...
    <script>
        ...
        $(document).ready(function () {
            $('#summernote{{$uruns->id}}').summernote({
                height: 300,
            });
        });
        @endforeach
    ...
    <script>
        @foreach($urun as $uruns)
        function readURL{{$uruns->id}}(input) {
            if (input.files && input.files[0]) {
                var reader = new FileReader();
                reader.onload = function (e) {
                    $('#imagePreview{{$uruns->id}}').css('background-image', 'url(' + e.target.result + ')');
                    $('#imagePreview{{$uruns->id}}').hide();
                    $('#imagePreview{{$uruns->id}}').fadeIn(650);
                }
                reader.readAsDataURL(input.files[0]);
            }
        }

        $("#imageUpload{{$uruns->id}}").change(function () {
            readURL{{$uruns->id}}(this);
        });
        @endforeach
    </script>
    <script>
    ...
    <script>
        ...
        @foreach($urun as $uruns)
        $(document).on("blur", '.baslik{{$uruns->id}}', function (event) {
            var value = $(this).val().toString().trim() , slug = convertToSlug(value).toString().trim();
            if (value) {
                $('.url{{$uruns->id}}').val(slug)
            }

        });

        $(document).on("blur", '.url{{$uruns->id}}', function (event) {
            var value = $(this).val().toString().trim() , slug = convertToSlug(value).toString().trim();
            if (value) {
                $(this).val(slug)
            }
        ...
        $('document').ready(function() {
            ...
            $(".bloc-text-image .delete").click(function() {
                ...
                $.ajax({
                    ...
                    success: function (d) {
                        toastr.success($(e.target).is(":checked") ? "Ürün Durumu Aktif" : "Ürün Durumu Pasif")
