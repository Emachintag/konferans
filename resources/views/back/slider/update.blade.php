                ...
                <div class="row">
                    <div class="col-sm-6">
                        ...
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">Slider Güncelleme</li>
                        </ol>
                    </div>
                        ...
                        <div class="card">
                            ...
                            <div class="card-body add-post">
                                ...
                                <form class="row needs-validation" method="post" autocomplete="off"
                                      enctype="multipart/form-data" action="{{route('slider.update',$slider[0]->lang_id)}}">
                                    @method('PUT')
                                    @csrf
                                    <div class="col-sm-12">
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                @foreach($slider as $sliders)
                                                    <div class="input-group-prepend">
                                                        <div class="input-group-text"><i style="margin: 10px" class="flag-icon flag-icon-{{$sliders->lang}}"></i>
                                                            <input value="{{$sliders->title}}"  name="title_{{$sliders->lang}}" class="form-control baslik{{$sliders->id}}" id="validationCustom01" type="text" placeholder="{{$sliders->langName}} Başlık Giriniz"></div>
                                                    </div>
                                                    <br>
                                                @endforeach
                                            </div>
                                            <div class="col-md-6">
                                                <label for="validationCustom01">Slider Alt Başlık:</label>
                                                @foreach($slider as $sliders)
                                                    <div class="input-group-prepend">
                                                        <div class="input-group-text"><i style="margin: 10px"
                                                                                         class="flag-icon flag-icon-{{$sliders->lang}}"></i>
                                                            <input value="{{$sliders->title_2}}" name="title_2_{{$sliders->lang}}" class="form-control"
                                                                   id="validationCustom01" type="text"
                                                                   placeholder="{{$sliders->langName}} Alt Başlık Giriniz"
                                                            ></div>
                                                    </div>
                                                    <br>
                                                @endforeach
                                                ...
                                                <div class="col-md-6">
                                                    <div class="avatar-upload">
                                                        <div class="avatar-edit">
                                                            <input name="image_{{$sliders->lang}}" type='file' id="imageUpload{{$sliders->id}}" accept=".png, .jpg, .jpeg" />
                                                            <label for="imageUpload{{$sliders->id}}"></label>
                                                        </div>
                                                        <i style="margin: 10px"
                                                           class="flag-icon flag-icon-{{$sliders->lang}}"></i>
                                                        <label>Referans Fotoğraf:</label>
                                                        <div class="avatar-preview">
                                                            <div id="imagePreview{{$sliders->id}}" style="background-image: url({{asset('public/'.$sliders->image)}});">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
...
@section('js')
    ...
    <script>
        @foreach($slider as $sliders)
        function readURL{{$sliders->id}}(input) {
            if (input.files && input.files[0]) {
                var reader = new FileReader();
                reader.onload = function (e) {
                    $('#imagePreview{{$sliders->id}}').css('background-image', 'url(' + e.target.result + ')');
                    $('#imagePreview{{$sliders->id}}').hide();
                    $('#imagePreview{{$sliders->id}}').fadeIn(650);
                }
                reader.readAsDataURL(input.files[0]);
            }
        }

        $("#imageUpload{{$sliders->id}}").change(function () {
            readURL{{$sliders->id}}(this);
        });
        @endforeach
    </script>
    <script>
    ...
    <script>
        ...
        @foreach($slider as $sliders)
        $(document).on("blur", '.baslik{{$sliders->id}}', function (event) {
            var value = $(this).val().toString().trim() , slug = convertToSlug(value).toString().trim();
            if (value) {
                $('.url{{$sliders->id}}').val(slug)
            }

        });

        $(document).on("blur", '.url{{$sliders->id}}', function (event) {
            var value = $(this).val().toString().trim() , slug = convertToSlug(value).toString().trim();
            if (value) {
                $(this).val(slug)
            }
