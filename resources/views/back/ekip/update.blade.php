                        ...
                        <div class="card">
                            ...
                            <div class="card-body add-post">
                                ...
                                <form class="row needs-validation" method="post" autocomplete="off"
                                      enctype="multipart/form-data" action="{{route('ekip.update',$ekip[0]->lang_id)}}">
                                    @method('PUT')
                                    @csrf
                                    <div class="col-sm-12">
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-6">
                                                <label for="validationCustom01">Ad Soyadı:</label>
                                                @foreach($ekip as $ekips)
                                                    <div class="input-group-prepend">
                                                        <div class="input-group-text"><i style="margin: 10px" class="flag-icon flag-icon-{{$ekips->lang}}"></i>
                                                            <input value="{{$ekips->title}}"  name="title_{{$ekips->lang}}" class="form-control baslik{{$ekips->id}}" id="validationCustom01" type="text" placeholder="{{$ekips->langName}} Ad Soyadı Giriniz"></div>
                                                    </div>
                                                    <br>
                                                @endforeach
                                            </div>
                                                ...
                                                @foreach($ekip as $ekips)
                                                    <div class="input-group-prepend">
                                                        <div class="input-group-text"><i style="margin: 10px"
                                                                                         class="flag-icon flag-icon-{{$ekips->lang}}"></i>
                                                            <input value="{{$ekips->title_2}}" name="title_2_{{$ekips->lang}}" class="form-control"
                                                                   id="validationCustom01" type="text"
                                                                   placeholder="{{$ekips->langName}} Ünvan Giriniz"
                                                            ></div>
                                                    </div>
                                                    <br>
                                                @endforeach
                                                ...
                                                <div class="col-md-6">
                                                    <div class="avatar-upload">
                                                        <div class="avatar-edit">
                                                            <input name="image_{{$ekips->lang}}" type='file' id="imageUpload{{$ekips->id}}" accept=".png, .jpg, .jpeg" />
                                                            <label for="imageUpload{{$ekips->id}}"></label>
                                                        </div>
                                                        <i style="margin: 10px"
                                                           class="flag-icon flag-icon-{{$ekips->lang}}"></i>
                                                        <label>Ekip Fotoğraf:</label>
                                                        <div class="avatar-preview">
                                                            <div id="imagePreview{{$ekips->id}}" style="background-image: url({{asset('public/'.$ekips->image)}});">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
...
@section('js')
    ...
    <script>
        @foreach($ekip as $ekips)
        function readURL{{$ekips->id}}(input) {
            if (input.files && input.files[0]) {
                var reader = new FileReader();
                reader.onload = function (e) {
                    $('#imagePreview{{$ekips->id}}').css('background-image', 'url(' + e.target.result + ')');
                    $('#imagePreview{{$ekips->id}}').hide();
                    $('#imagePreview{{$ekips->id}}').fadeIn(650);
                }
                reader.readAsDataURL(input.files[0]);
            }
        }

        $("#imageUpload{{$ekips->id}}").change(function () {
            readURL{{$ekips->id}}(this);
        });
        @endforeach
    </script>
    <script>
