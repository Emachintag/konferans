                        ...
                        <div class="card">
                            ...
                            <div class="card-body add-post">
                                ...
                                <form class="row needs-validation" method="post" autocomplete="off"
                                      enctype="multipart/form-data" action="{{route('yorum.update',$yorum[0]->lang_id)}}">
                                    @method('PUT')
                                    @csrf
                                    ...
                                    <div class="col-sm-12">
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                @foreach($yorum as $yorums)

                                                    <div class="input-group-prepend">
                                                        <div class="input-group-text"><i style="margin: 10px"
                                                                                         class="flag-icon flag-icon-{{$yorums->lang}}"></i>
                                                            <input  value="{{$yorums->title}}"  name="title_{{$yorums->lang}}" class="form-control baslik{{$yorums->id}}"
                                                                    id="validationCustom01" type="text"
                                                                    placeholder="{{$yorums->langName}} İsim Giriniz"
                                                            ></div>
                                                    </div>
                                                    <br>
                                            ...
                                            <div class="col-md-6">
                                                <label for="validationCustom01">Ünvan:</label>
                                                @foreach($yorum as $yorums)
                                                    <div class="input-group-prepend">
                                                        <div class="input-group-text"><i style="margin: 10px"
                                                                                         class="flag-icon flag-icon-{{$yorums->lang}}"></i>
                                                            <input value="{{$yorums->title_2}}" name="title_2_{{$yorums->lang}}" class="form-control"
                                                                   id="validationCustom01" type="text"
                                                                   placeholder="{{$yorums->langName}} Ünvan Giriniz"
                                                            ></div>
                                                    </div>
                                                    <br>
                                                @endforeach
                                                ...
                                                <div class="col-md-6">
                                                    <div class="avatar-upload">
                                                        <div class="avatar-edit">
                                                            <input name="image_{{$yorums->lang}}" type='file' id="imageUpload{{$yorums->id}}" accept=".png, .jpg, .jpeg" />
                                                            <label for="imageUpload{{$yorums->id}}"></label>
                                                        </div>
                                                        <i style="margin: 10px"
                                                           class="flag-icon flag-icon-{{$yorums->lang}}"></i>
                                                        <label>Yorum Fotoğraf:</label>
                                                        <div class="avatar-preview">
                                                            <div id="imagePreview{{$yorums->id}}" style="background-image: url({{asset('public/'.$yorums->image)}});">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="theme-form">
                                                    <div class="form-group">
                                                        <i style="margin: 10px"
                                                           class="flag-icon flag-icon-{{$yorums->lang}}"></i>
                                                        <label>İçerik:</label>
                                                        <textarea id="summernote{{$yorums->id}}"
                                                                  name="text_{{$yorums->lang}}">{!! $yorums->text !!}</textarea>
                                                    </div>
                                                </div>
                                            </div>
                                        @endforeach
                                    ...
                                    <input type="hidden" name="id" value="{{$yorum[0]->lang_id}}" >
...
@section('js')
    ...
    <script>
        ...
        $(document).ready(function () {
            $('#summernote{{$yorums->id}}').summernote({
                height: 300,
            });
        });
        @endforeach
    ...
    <script>
        @foreach($yorum as $yorums)
        function readURL{{$yorums->id}}(input) {
            if (input.files && input.files[0]) {
                var reader = new FileReader();
                reader.onload = function (e) {
                    $('#imagePreview{{$yorums->id}}').css('background-image', 'url(' + e.target.result + ')');
                    $('#imagePreview{{$yorums->id}}').hide();
                    $('#imagePreview{{$yorums->id}}').fadeIn(650);
                }
                reader.readAsDataURL(input.files[0]);
            }
        }

        $("#imageUpload{{$yorums->id}}").change(function () {
            readURL{{$yorums->id}}(this);
        });
        @endforeach
    </script>
    <script>
    ...
    <script>
        ...
        @foreach($yorum as $yorums)
        $(document).on("blur", '.baslik{{$yorums->id}}', function (event) {
            var value = $(this).val().toString().trim() , slug = convertToSlug(value).toString().trim();
            if (value) {
                $('.url{{$yorums->id}}').val(slug)
            }

        });

        $(document).on("blur", '.url{{$yorums->id}}', function (event) {
            var value = $(this).val().toString().trim() , slug = convertToSlug(value).toString().trim();
            if (value) {
                $(this).val(slug)
            }
        ...
        $('document').ready(function() {
            ...
            $(".bloc-text-image .delete").click(function() {
                ...
                $.ajax({
                    ...
                    success: function (d) {
                        toastr.success($(e.target).is(":checked") ? "Yorum Durumu Aktif" : "Yorum Durumu Pasif")
