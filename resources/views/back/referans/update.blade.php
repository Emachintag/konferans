                ...
                <div class="row">
                    <div class="col-sm-6">
                        ...
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">Referans Günceller</li>
                        </ol>
                    </div>
                        ...
                        <div class="card">
                            ...
                            <div class="card-body add-post">
                                ...
                                <form class="row needs-validation" method="post" autocomplete="off"
                                      enctype="multipart/form-data" action="{{route('referans.update',$referans[0]->lang_id)}}">
                                    @method('PUT')
                                    @csrf
                                    <div class="col-sm-12">
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                @foreach($referans as $referanslar)
                                                    <div class="input-group-prepend">
                                                        <div class="input-group-text"><i style="margin: 10px" class="flag-icon flag-icon-{{$referanslar->lang}}"></i>
                                                            <input value="{{$referanslar->title}}"  name="title_{{$referanslar->lang}}" class="form-control baslik{{$referanslar->id}}" id="validationCustom01" type="text" placeholder="{{$referanslar->langName}} Referans Adı Giriniz"></div>
                                                    </div>
                                                    <br>
                                                @endforeach
                                            </div>
                                                ...
                                                @foreach($referans as $referanslar)
                                                    <div class="input-group-prepend">
                                                        <div class="input-group-text"><i style="margin: 10px"
                                                                                         class="flag-icon flag-icon-{{$referanslar->lang}}"></i>
                                                            <input value="{{$referanslar->link}}" name="link_{{$referanslar->lang}}" class="form-control"
                                                                   id="validationCustom01" type="text"
                                                                   placeholder="{{$referanslar->langName}} Referans Linki Giriniz"
                                                            ></div>
                                                    </div>
                                                    <br>
                                                @endforeach
                                                ...
                                                <div class="col-md-6">
                                                    <div class="avatar-upload">
                                                        <div class="avatar-edit">
                                                            <input name="image_{{$referanslar->lang}}" type='file' id="imageUpload{{$referanslar->id}}" accept=".png, .jpg, .jpeg" />
                                                            <label for="imageUpload{{$referanslar->id}}"></label>
                                                        </div>
                                                        <i style="margin: 10px"
                                                           class="flag-icon flag-icon-{{$referanslar->lang}}"></i>
                                                        <label>Referans Fotoğraf:</label>
                                                        <div class="avatar-preview">
                                                            <div id="imagePreview{{$referanslar->id}}" style="background-image: url({{asset('public/'.$referanslar->image)}});">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
...
@section('js')
    ...
    <script>
        @foreach($referans as $referanslar)
        function readURL{{$referanslar->id}}(input) {
            if (input.files && input.files[0]) {
                var reader = new FileReader();
                reader.onload = function (e) {
                    $('#imagePreview{{$referanslar->id}}').css('background-image', 'url(' + e.target.result + ')');
                    $('#imagePreview{{$referanslar->id}}').hide();
                    $('#imagePreview{{$referanslar->id}}').fadeIn(650);
                }
                reader.readAsDataURL(input.files[0]);
            }
        }

        $("#imageUpload{{$referanslar->id}}").change(function () {
            readURL{{$referanslar->id}}(this);
        });
        @endforeach
    </script>
    <script>
    ...
    <script>
        ...
        @foreach($referans as $referanslar)
        $(document).on("blur", '.baslik{{$referanslar->id}}', function (event) {
            var value = $(this).val().toString().trim() , slug = convertToSlug(value).toString().trim();
            if (value) {
                $('.url{{$referanslar->id}}').val(slug)
            }

        });

        $(document).on("blur", '.url{{$referanslar->id}}', function (event) {
            var value = $(this).val().toString().trim() , slug = convertToSlug(value).toString().trim();
            if (value) {
                $(this).val(slug)
            }
