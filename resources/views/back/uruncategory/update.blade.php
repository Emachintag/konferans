                        ...
                        <div class="card">
                            ...
                            <div class="card-body add-post">
                                ...
                                <form class="row needs-validation" method="post" autocomplete="off"
                                      enctype="multipart/form-data" action="{{route('urun-category.update',$uruncategory[0]->lang_id)}}">
                                    @method('PUT')
                                    @csrf
                                    <div class="col-sm-12">
                                                ...
                                                <div>
                                                    ...
                                                    <select class="js-example-basic-single col-sm-12 select2-hidden-accessible" tabindex="-1" aria-hidden="true" name="kategori_tree">
                                                        <optgroup label="Kategoriler">
                                                            @if($uruncategory[0]->kategori_tree == 0)
                                                                <option selected value="0">Genel</option>
                                                                @foreach($uruncategorysingle as $uruncategorysingles)
                                                                    <option  value="{{$uruncategorysingles->id}}">{{$uruncategorysingles->kategori}}</option>
                                                                @endforeach
                                                            @else
                                                                <option value="0">Genel</option>
                                                                @foreach($uruncategorysingle as $uruncategorysingles)
                                                                    <option @if($uruncategory[0]->lang_id == $uruncategorysingles->lang_id) selected @endif  value="{{$uruncategorysingles->id}}">{{$uruncategorysingles->kategori}}</option>
                                                                @endforeach
                                                            @endif
                                                        </optgroup>
                                                    </select>
                                                    ...
                                                    <div class="col-md-6">
                                                        <div class="avatar-upload">
                                                            ...
                                                            <div class="avatar-preview">
                                                                <div id="imagePreview{{$uruncategorys->id}}" style="background-image: url({{asset('public/'.$uruncategorys->image)}});">
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
...
@section('js')
    ...
    <script>
        @foreach($uruncategory as $uruncategorys)
        function readURL{{$uruncategorys->id}}(input) {
