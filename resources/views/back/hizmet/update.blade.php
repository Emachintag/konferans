                        ...
                        <div class="card">
                            ...
                            <div class="card-body add-post">
                                ...
                                <form class="row needs-validation" method="post" autocomplete="off"
                                      enctype="multipart/form-data" action="{{route('hizmet.update',$hizmet[0]->lang_id)}}">
                                    @method('PUT')
                                    @csrf
                                    ...
                                    <div class="col-sm-12">
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                @foreach($hizmet as $hizmetler)

                                                    <div class="input-group-prepend">
                                                        <div class="input-group-text"><i style="margin: 10px"
                                                                                         class="flag-icon flag-icon-{{$hizmetler->lang}}"></i>
                                                            <input  value="{{$hizmetler->title}}"  name="title_{{$hizmetler->lang}}" class="form-control baslik{{$hizmetler->id}}"
                                                                    id="validationCustom01" type="text"
                                                                    placeholder="{{$hizmetler->langName}} Başlık Giriniz"
                                                            ></div>
                                                    </div>
                                                    <br>
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                @foreach($hizmet as $hizmetler)
                                                    <div class="input-group-prepend">
                                                        <div class="input-group-text"><i style="margin: 10px"
                                                                                         class="flag-icon flag-icon-{{$hizmetler->lang}}"></i>
                                                            <input value="{{$hizmetler->url}}" name="url_{{$hizmetler->lang}}" class="form-control url{{$hizmetler->id}}"
                                                                   id="slug-target" type="text"
                                                                   placeholder="{{$hizmetler->langName}} URL Alanı Otomatik Oluşturuluyor"
                                                            ></div>
                                                    </div>
                                                    <br>
                                                @endforeach
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                @foreach($hizmet as $hizmetler)
                                                    <div class="input-group-prepend">
                                                        <div class="input-group-text"><i style="margin: 10px"
                                                                                         class="flag-icon flag-icon-{{$hizmetler->lang}}"></i>
                                                            <input value="{{$hizmetler->title_2}}" name="title_2_{{$hizmetler->lang}}" class="form-control"
                                                                   id="validationCustom01" type="text"
                                                                   placeholder="{{$hizmetler->langName}} Alt Başlık Giriniz"
                                                            ></div>
                                                    </div>
                                                    <br>
                                                @endforeach
                                            ...
                                            <div class="col-md-6">
                                                <div>
                                                    ...
                                                    <select
                                                        ...
                                                        <optgroup label="Kategoriler">
                                                            @foreach($hizmetcategorysingle as $hizmetcategorysingles)
                                                                <option @if($$hizmetler->lang_id == $hizmetcategorysingles->lang_id) selected @endif value="{{$hizmetcategorysingles->id}}">{{$hizmetcategorysingles->kategori}}</option>
                                                            @endforeach
                                                        </optgroup>
                                                    </select>
                                                </div>
                                                ...
                                                <div class="input-group-prepend">
                                                    <div class="input-group-text">
                                                        <input value="{{$hizmetler->sira}}" name="sira" class="form-control" id="validationCustom01" type="number" placeholder="Sıra Giriniz">
                                                    </div>
                                                </div>
                                            </div>
                                                ...
                                                <div class="col-md-6">
                                                    <div class="avatar-upload">
                                                        <div class="avatar-edit">
                                                            <input name="image_{{$hizmetler->lang}}" type='file' id="imageUpload{{$hizmetler->id}}" accept=".png, .jpg, .jpeg" />
                                                            <label for="imageUpload{{$hizmetler->id}}"></label>
                                                        </div>
                                                        <i style="margin: 10px"
                                                           class="flag-icon flag-icon-{{$hizmetler->lang}}"></i>
                                                        <label>Haber Kapak Fotoğraf:</label>
                                                        <div class="avatar-preview">
                                                            <div id="imagePreview{{$hizmetler->id}}" style="background-image: url({{asset('public/'.$hizmetler->image)}});">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="theme-form">
                                                    <div class="form-group">
                                                        <i style="margin: 10px"
                                                           class="flag-icon flag-icon-{{$hizmetler->lang}}"></i>
                                                        <label>İçerik:</label>
                                                        <textarea id="summernote{{$hizmetler->id}}"
                                                                  name="text_{{$hizmetler->lang}}">{!! $hizmetler->text !!}</textarea>
                                                    </div>
                                                </div>
                                            </div>
                                        @endforeach
                                    ...
                                    @foreach(DB::table('hizmet_gorsel')->where('hizmet_id',$hizmet[0]->lang_id)->get() as $images)
                                    ...
                                    <input type="hidden" name="id" value="{{$hizmet[0]->lang_id}}" >
...
@section('js')
    ...
    <script>
        ...
        $(document).ready(function () {
            $('#summernote{{$hizmetler->id}}').summernote({
                height: 300,
            });
        });
        @endforeach
    ...
    <script>
        @foreach($hizmet as $hizmetler)
        function readURL{{$hizmetler->id}}(input) {
            if (input.files && input.files[0]) {
                var reader = new FileReader();
                reader.onload = function (e) {
                    $('#imagePreview{{$hizmetler->id}}').css('background-image', 'url(' + e.target.result + ')');
                    $('#imagePreview{{$hizmetler->id}}').hide();
                    $('#imagePreview{{$hizmetler->id}}').fadeIn(650);
                }
                reader.readAsDataURL(input.files[0]);
            }
        }

        $("#imageUpload{{$hizmetler->id}}").change(function () {
            readURL{{$hizmetler->id}}(this);
        });
        @endforeach
    </script>
    <script>
    ...
    <script>
        ...
        @foreach($hizmet as $hizmetler)
        $(document).on("blur", '.baslik{{$hizmetler->id}}', function (event) {
            var value = $(this).val().toString().trim() , slug = convertToSlug(value).toString().trim();
            if (value) {
                $('.url{{$hizmetler->id}}').val(slug)
            }

        });

        $(document).on("blur", '.url{{$hizmetler->id}}', function (event) {
            var value = $(this).val().toString().trim() , slug = convertToSlug(value).toString().trim();
            if (value) {
                $(this).val(slug)
            }
