    ...
    <div class="page-body">
        <div class="container-fluid">
            <div class="page-header">
                <div class="row">
                    <div class="col-sm-6">
                        <h3>Misyon Sayfası</h3>
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{route('index')}}">Aansayfa</a></li>
                            <li class="breadcrumb-item">Misyon</li>
                            <li class="breadcrumb-item active">Misyon ekle</li>
                        </ol>
                    </div>
                        ...
                        <div class="card">
                            ...
                            <div class="card-body add-post">
                                ...
                                <form class="row needs-validation" method="post" autocomplete="off"
                                      enctype="multipart/form-data" action="{{route('misyon.update',1635233954)}}">
                                    @method('PUT')
                                    @csrf
                                    <div class="col-sm-12">
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-12">
                                                ...
                                                @foreach($misyon as $misyonn)
                                                    <div class="input-group-prepend">
                                                        <div class="input-group-text"><i style="margin: 10px" class="flag-icon flag-icon-{{$misyonn->lang}}"></i>
                                                            <input value="{{$misyonn->title}}" name="title_{{$misyonn->lang}}" class="form-control baslik{{$misyonn->id}}" id="validationCustom01" type="text" placeholder="{{$misyonn->langName}} Başlık Giriniz"></div>
                                                    </div>
                                                    <br>
                                                @endforeach
                                            </div>
                                                ...
                                                <div class="col-md-6">
                                                    <div class="avatar-upload">
                                                        <div class="avatar-edit">
                                                            <input name="image_{{$misyonn->lang}}" type='file' id="imageUpload{{$misyonn->id}}" accept=".png, .jpg, .jpeg" />
                                                            <label for="imageUpload{{$misyonn->id}}"></label>
                                                        </div>
                                                        <i style="margin: 10px"
                                                           class="flag-icon flag-icon-{{$misyonn->lang}}"></i>
                                                        <label>Misyon Kapak Fotoğraf:</label>
                                                        <div class="avatar-preview">
                                                            <div id="imagePreview{{$misyonn->id}}" style="background-image: url({{asset('public/'.$misyonn->image)}});">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="theme-form">
                                                    <div class="form-group">
                                                        <i style="margin: 10px"
                                                           class="flag-icon flag-icon-{{$misyonn->lang}}"></i>
                                                        <label>İçerik:</label>
                                                        <textarea id="summernote{{$misyonn->id}}"
                                                                  name="text_{{$misyonn->lang}}">{!! $misyonn->text !!}</textarea>
                                                    </div>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
                                    @foreach(DB::table('misyon_gorsel')->where('misyon_id',1635233954)->get() as $images)
...
@section('js')
    ...
    <script>
        ...
        $(document).ready(function () {
            $('#summernote{{$misyonn->id}}').summernote({
                height: 300,
            });
        });
        @endforeach
    ...
    <script>
        @foreach($misyon as $misyonn)
        function readURL{{$misyonn->id}}(input) {
            if (input.files && input.files[0]) {
                var reader = new FileReader();
                reader.onload = function (e) {
                    $('#imagePreview{{$misyonn->id}}').css('background-image', 'url(' + e.target.result + ')');
                    $('#imagePreview{{$misyonn->id}}').hide();
                    $('#imagePreview{{$misyonn->id}}').fadeIn(650);
                }
                reader.readAsDataURL(input.files[0]);
            }
        }

        $("#imageUpload{{$misyonn->id}}").change(function () {
            readURL{{$misyonn->id}}(this);
        });
        @endforeach
    </script>
    <script>
